...
plot(x = heart_disease_data_1$age, y = heart_disease_data_1$thalach,
     pch = 19,col = colors[factor(heart_disease_data_1$cp)],
...
type0 <- subset(heart_disease_data_1, cp == 0)
type1 <- subset(heart_disease_data_1, cp == 1)
type2 <- subset(heart_disease_data_1, cp == 2)
type3 <- subset(heart_disease_data_1, cp == 3)
...
```{r,echo=FALSE}
mod1 = lm(thalach~age+as.factor(cp), data=heart_disease_data_1)
summary(mod1)
```
...
```{r,echo=FALSE}
mod_int = lm(thalach~age+as.factor(cp)+age*as.factor(cp),data=heart_disease_data_1)
summary(mod_int)
```
...
colors2 <- c("blue", 
            "#DE3289")
plot(x = heart_disease_data_1$age, y = heart_disease_data_1$thalach,
     pch = 19,col = colors2[factor(heart_disease_data_1$sex)],
     xlab = "Age", ylab = "Maximum Heart Rate Achieve",
     main = "Relationship between Age and Maximum Heart Rate ")
legend("topright",c("female","male"), pch = 19,col = colors2)

female <- subset(heart_disease_data_1, sex == 0)
male <- subset(heart_disease_data_1, sex == 1)
abline(lm(thalach~age, data = female),col="blue")
abline(lm(thalach~age, data = male),col="#DE3289")
```

```{r,echo=FALSE}
mod_2 = lm(thalach~age+as.factor(sex),data=heart_disease_data_1)
summary(mod_2)
```

```{r,echo=FALSE}
mod_int_2 = lm(thalach~age+as.factor(sex)+age*as.factor(sex),data=heart_disease_data_1)
summary(mod_int_2)
```
...
heart_disease_data_1_shuffle<-heart_disease_data_1[sample(nrow(heart_disease_data_1)),]

#Create 10 equally size folds
folds <- cut(seq(1,nrow(heart_disease_data_1_shuffle)),breaks=5,labels=FALSE)
```


```{r}

#folds<-createFolds(heart_disease_data_1,k=5)
...
for(i in 1:5){
    ...
    testData <- heart_disease_data_1_shuffle[testIndexes, ]
    trainData <- heart_disease_data_1_shuffle[-testIndexes, ]
