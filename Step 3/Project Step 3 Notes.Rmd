...
colors <- c("#FDAE61", 
            ...
            "#F90123",
            "blue")
plot(x = train_data$age, y = train_data$thalach,
     pch = 19,col = colors[factor(train_data$cp)],
...
type0 <- subset(train_data, cp == 0)
type1 <- subset(train_data, cp == 1)
type2 <- subset(train_data, cp == 2)
type3 <- subset(train_data, cp == 3)
...
```{r,echo=FALSE}
mod1 = lm(thalach~age+as.factor(cp), data=train_data)
summary(mod1)
```
...
```{r,echo=FALSE}
mod_int = lm(thalach~age+as.factor(cp)+age*as.factor(cp),data=train_data)
summary(mod_int)
```
...
Another model that we are interested 
