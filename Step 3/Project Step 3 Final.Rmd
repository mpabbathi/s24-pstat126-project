...
$$
\text{max heart rate}_i = \beta_0 + \beta_1age_i + \beta_2cp_i  + \varepsilon_i
$$

The theoretical expected response should looks like: 

$$
\mathbb{E}[\text{max heart rate}_i] = \begin{cases}\beta_0 + \beta_1 age_i  &\text{if } cp = 0 \\ 
...
Hypothetically, if the different chest pain types don't make significant differences in response, the intercept should not shift a lot. We can first visualize this relation in the following plots: 
...
From the graph, it can be seen that the best-fit line is shifting a lot, so it is highly likely for `chest pain type` to have significant association with `maximum heart rate` after taking care of `age`.
...
Another model that we are interested in is the relationship between `max heart rate`, `age`, and `sex`. We first visualize the relationship.
...
From the graph, we can see that the two best fit lines have a distinct trend, which could be a sign that `sex` is significantly related to `max heart rate` after taking care of `age`. In that case, we decided to analyze it as our second model and examine the interaction between variables.
...
Criteria:
 - AIC - which is adding variables with the lowest AIC value, starting with an intercept-only model.
...
By going through forward step-wise selection, we have added 7 predictors for the model, which are `slope`, `age`, `cp`, `exang`, `target`, `trestbps` and `restecg`. 
